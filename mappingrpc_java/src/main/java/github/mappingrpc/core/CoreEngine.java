...
/**
 * 同个client instance 里也可设定 isListenEngine = false 的providerList，
 * 因为 feature 是 two-way rpc，即server 也可主动调用client的功能，但client不需listenPort
...
public class CoreEngine implements BeanNameAware, Closeable {
	...
	public void start() {
		...
		if (isListenEngine) {
			listenServer = new MappingPackageServer(metaHolder, listenPort, tlsConfig);
			listenServer.start();
		} else {
			siteConfig.put(SiteConfigConstant.client_connectionName, this.beanName);

			connectServer = new MappingPackageClient(metaHolder, serverList, tlsConfig);
	...
	public Object createConsumerProxy(String serviceInterface) {
		...
		if (proxyHandler == null) {// FIXME
			// proxyHandler = new ServiceInvocationHandler(connectServer); FIXME
